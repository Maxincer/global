...
class DatabaseBasicInfo:
    def __init__(self):
        ...
        self.col_strategy_info = db_basicinfo['strategy_info']
    ...
    def update_acctinfo(self):
        ...
        for dict_to_be_inserted in list_dicts_to_be_inserted:
            if dict_to_be_inserted['PreTradeDataFilePath']:
                str_ = dict_to_be_inserted['PreTradeDataFilePath'].replace("'", '"')
                dict_pretrd_fpath = json.loads(str_)
                if 'PrivateSecurityLoanMarketData' not in dict_pretrd_fpath:
                    dict_pretrd_fpath['PrivateSecurityLoanMarketData'] = None
                if 'PublicSecurityLoanMarketData' not in dict_pretrd_fpath:
                    dict_pretrd_fpath['PublicSecurityLoanMarketData'] = None
                dict_to_be_inserted['PreTradeDataFilePath'] = dict_pretrd_fpath
            else:
                dict_to_be_inserted['PreTradeDataFilePath'] = {
                    'PublicSecurityLoanMarketData': None,
                    'PrivateSecurityLoanMarketData': None
                }
    ...
    def update_strategy_info(self):
        ...
        无运行顺序要求
        """
        df_strategy_info = pd.read_excel(self.fpath_basicinfo, sheet_name='strategy_info')
        df_strategy_info = df_strategy_info.where(df_strategy_info.notnull(), None)
        list_dicts_to_be_inserted = df_strategy_info.to_dict('records')
        for dict_to_be_inserted in list_dicts_to_be_inserted:
            dict_to_be_inserted['DataDate'] = self.str_today
        self.col_strategy_info.delete_many({'DataDate': self.str_today})
        self.col_strategy_info.insert_many(list_dicts_to_be_inserted)
        print('Collection "strategy_info" has been updated.')
